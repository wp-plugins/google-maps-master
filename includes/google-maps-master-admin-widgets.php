<?php
if( is_multisite() ) {
	function menu_multi_google_maps_admin_widgets(){
	// Create menu
	add_submenu_page( 'google-maps-master', 'Widgets', 'Widgets', 'manage_options', 'google-maps-master-admin-widgets', 'google_maps_master_admin_widgets' );
	}
}
else {
	// Create menu
	function menu_single_google_maps_admin_widgets(){
		if ( is_admin() )
		add_submenu_page( 'google-maps-master', 'Widgets', 'Widgets', 'manage_options', 'google-maps-master-admin-widgets', 'google_maps_master_admin_widgets' );
	}
}

function google_maps_master_admin_widgets(){
?>
<div class="wrap">
...
<h2><b>&nbsp;Widgets</b></h2>
<?php
if(!class_exists('google_maps_master_admin_widgets_table')){
	require_once( dirname( __FILE__ ) . '/google-maps-master-admin-widgets-table.php');
}
//Prepare Table of elements
$wp_list_table = new google_maps_master_admin_widgets_table();
//Table of elements
...
if( is_multisite() ) {
add_action( 'network_admin_menu', 'menu_multi_google_maps_admin_widgets' );
}
else {
add_action( 'admin_menu', 'menu_single_google_maps_admin_widgets' );
}
?>